import React from 'react'
import { RED_TILE_PATTERN } from './libs/SVGTileFills'
import { BLACK, BLUE_DARK, RED } from '../libs/UIConstants'
import RedCRTBlur from './libs/RedCRTBlur'

const IconClose = () => {
  return (
    <div className='cursor-pointer relative left-1.5'>
      <svg width='44' height='34' viewBox='-1 0 44 34'>
        <rect x='0' y='0' width='34' height='34' fill={BLUE_DARK} fillOpacity='0.85' />
        <rect x='0' y='0' width='34' height='34' fill={RED} fillOpacity='0.1' />
        <rect x='0' y='0' width='34' height='34' fill='url(#redTile)' />
        <RedCRTBlur />
        <g filter='url(#RedCRTBlur1) url(#RedCRTBlur2)'>
          <rect x='0' y='0' width='34' height='34' stroke={RED} strokeWidth='2' fillOpacity='0' />
          <path
            d='M23.338 8H25.9746L18.3183 17L26 26H23.338L17 18.575L10.6366 26H8L15.6563 17L8 8H10.6366L17 15.425L23.338 8Z'
            fill={RED}
          />
        </g>
        {RED_TILE_PATTERN}
      </svg>
    </div>
  )
}

export default IconClose
